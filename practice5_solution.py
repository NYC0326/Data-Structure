...
class BinarySearchTree:
	...
	def findMin(self):
		if self.isEmpty(): # 만약 노드가 비어있으면 None을 반환
			return None
		p = self.root # 그게 아니면 p를 루트라고 하고
		while p.left: # 최솟값이니 계속 왼쪽을 탐색함. 없으면 반복문 끝남
			p = p.left
		return p # 그걸 반환
		
	# Return the node with the maximum value 
	def findMax(self): # 만약 노드가 비어있으면 None을 반환
		if self.isEmpty():
			return None
		p = self.root # 그게 아니면 p를 루트라고 하고
		while p.right: # 최댓값이니 계속 오른쪽을 탐색함. 없으면 반복문 끝남
			p = p.right
		return p # 그걸 반환
